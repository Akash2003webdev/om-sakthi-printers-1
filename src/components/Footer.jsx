        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="body-sm mb-6" style={{ maxWidth: '22rem' }}>
              Crafting premium print experiences for over 18 years. From royal wedding invitations to professional business stationery — every print tells your story.
            </p>
          ...
          {/* Nav col */}
          <div>
            <h4 className="label mb-5" style={{ color: 'rgba(201,136,16,0.7)' }}>Pages</h4>
            <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {[{to:'/',label:'Home'},{to:'/gallery',label:'Gallery'},{to:'/services',label:'Services'},{to:'/about',label:'About Us'},{to:'/contact',label:'Contact'}].map(l => (
                <li key={l.to}>
          ...
          {/* Contact col */}
          <div>
            <h4 className="label mb-5" style={{ color: 'rgba(201,136,16,0.7)' }}>Contact</h4>
            <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[
                ...
                { icon: Clock,  content: 'Mon–Sat: 9AM–8PM · Sun: 10AM–2PM', href: null },
      ...
      <div style={{ borderTop: '1px solid var(--border)' }}>
        <div className="container-xl" style={{ paddingTop: '1.25rem', paddingBottom: '1.25rem', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem' }}>
          <p className="label" style={{ color: 'var(--text4)', letterSpacing: '0.03em', fontWeight: 400, fontSize: '0.75rem', textTransform: 'none' }}>
            © {year} Om Sakthi Printers, Sattur, Tamil Nadu. All rights reserved.
          </p>
          <div style={{ display: 'flex', gap: '1.5rem' }}>
            {['Wedding Card Printing Sattur', 'Best Printers Sattur'].map((t, i) => (
              <span key={i} style={{ fontSize: '0.6875rem', color: 'var(--text4)', opacity: 0.6 }}>{t}</span>
            ))}
          </div>
        </div>
      </div>
